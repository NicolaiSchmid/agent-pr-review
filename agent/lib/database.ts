...
export const store = {
  ...
  completeCiTask: (v: Record<string, unknown>) => convex().mutation(mutation("completeCiTask"), args(v)) as Promise<boolean>,
  claimResultPost: <T>(taskId: string, leaseToken: string) => convex().mutation(mutation("claimResultPost"), args({ taskId, leaseToken })) as Promise<T>,
  recordResultComment: (taskId: string, leaseToken: string, commentId: number, body: string) => convex().mutation(mutation("recordResultComment"), args({ taskId, leaseToken, commentId, body })) as Promise<boolean>,
  replaceMissingResultComment: (taskId: string, leaseToken: string, commentId: number) => convex().mutation(mutation("replaceMissingResultComment"), args({ taskId, leaseToken, commentId })) as Promise<boolean>,
  ...
  claimCompletedForRevalidation: <T>(limit: number) => convex().mutation(mutation("claimCompletedForRevalidation"), args({ limit })) as Promise<T>,
  claimRerunCleanup: <T>(limit: number) => convex().mutation(mutation("claimRerunCleanup"), args({ limit })) as Promise<T>,
  ...
  rerunDisposition: (taskId: string) => convex().query(query("rerunDisposition"), args({ taskId })) as Promise<{ result_state: "reopened" | "superseded" | "cancelled" | "completed" | "publishing"; conclusion: "success" | "failure" | null; body: string | null; comment_id: number | null; lease_token: string | null } | null>,
