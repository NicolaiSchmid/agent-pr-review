...
export const store = {
  ...
  recordResultComment: (taskId: string, leaseToken: string, commentId: number) => convex().mutation(mutation("recordResultComment"), args({ taskId, leaseToken, commentId })) as Promise<boolean>,
  ...
  rerunDisposition: (taskId: string) => convex().query(query("rerunDisposition"), args({ taskId })) as Promise<{ result_state: "reopened" | "superseded" | "cancelled" | "completed"; conclusion: "success" | "failure" | null } | null>,
