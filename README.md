...
- Slack mentions, DMs, threaded continuation, and interactive approvals through Vercel Connect: implemented for conversation and user-scoped memory. Repository tools require a deployment-provisioned verified Slack-to-GitHub identity link in the Convex `principalIdentities` table containing `provider: "github"`, the lowercase target repository owner (or the connector's installation ID) in `providerTenantId`, the stable GitHub ID in `providerUserId`, the current login in `providerLogin`, the Slack caller's Eve principal in `principalId`, and a numeric `verifiedAt` timestamp. Authorization selects only links matching the target owner or trusted installation; an in-agent linking flow is not yet implemented.
