...
export const rerunDisposition = queryGeneric({args:{secret:v.string(),taskId:v.string()},handler:async({db},a)=>{authorize(a.secret);const t:any=await db.query("tasks").withIndex("by_external_id",q=>q.eq("externalId",a.taskId)).unique();if(!t)return null;return t.state==="cancelled"?"cancelled":t.state==="superseded"?"superseded":"reopened";}});
