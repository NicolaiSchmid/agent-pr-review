...
export const claimCompletedForRevalidation = mutationGeneric({
  ...
  handler: async ({ db }, a) => {
    ...
    const out = [];
    for (const t of rows) {
      if (t.kind !== "pr_review" || !t.headSha) continue;
      ...
      out.push({
        ...
        pull_request_number: c.pullRequestNumber,
      });
    }
    return out;
  },
