...
export const deferCi = mutationGeneric({
  ...
  handler: async ({ db }, a) => {
    ...
    await db.insert("tasks", { externalId: a.taskId, conversationId: c._id, kind: "pr_review", state: "waiting_for_ci", repositoryId: a.repositoryId, headSha: a.headSha, conversationHead, repositoryHead, updatedAt: now }); return a.taskId;
  },
});
...
export const claimDeferred = mutationGeneric({
  ...
  handler: async ({ db }, a) => { authorize(a.secret); const waiting: any[] = await db.query("tasks").withIndex("by_state_updated",q=>q.eq("state","waiting_for_ci")).take(a.limit); const reviewing: any[] = (await db.query("tasks").withIndex("by_state_updated",q=>q.eq("state","reviewing")).take(a.limit)).filter((t:any)=>t.updatedAt<a.staleBefore); const publishing: any[] = (await db.query("tasks").withIndex("by_state_updated",q=>q.eq("state","publishing")).take(a.limit)).filter((t:any)=>t.updatedAt<a.staleBefore); const candidates=[...waiting,...reviewing,...publishing].filter(t=>t.headSha&&!t.rerunCleanupPending).sort((x,y)=>x.updatedAt-y.updatedAt||x._creationTime-y._creationTime).slice(0,a.limit); const out=[]; for (const t of candidates) { const c: any = await db.get(t.conversationId); if (!c?.repositoryId || !c.repositoryOwner || !c.repositoryName || !c.pullRequestNumber) continue; const leaseToken=crypto.randomUUID(); await db.patch(t._id,{state:"reviewing",leaseToken,updatedAt:Date.now()}); out.push({id:t.externalId,head_sha:t.headSha,repository_id:c.repositoryId,repository_owner:c.repositoryOwner,repository_name:c.repositoryName,github_installation_id:c.githubInstallationId??null,pull_request_number:c.pullRequestNumber,lease_token:leaseToken}); } return out; },
});
...
export const releaseOperationPull = mutationGeneric({args:{secret:v.string(),id:v.string(),number:v.number()},handler:async({db},a)=>{authorize(a.secret);const o:any=await db.query("changeOperations").withIndex("by_external_id",q=>q.eq("externalId",a.id)).unique();if(!o||o.pullRequestNumber!==a.number)return false;await db.patch(o._id,{pullRequestNumber:undefined,updatedAt:Date.now()});return true;}});

export const rerun = mutationGeneric({
  args:{secret:v.string(),repositoryId:v.string(),headSha:v.string()},
  handler:async({db},a)=>{authorize(a.secret);const repositoryHead=`${a.repositoryId}\u0000${a.headSha}`;const rows:any[]=await db.query("tasks").withIndex("by_repository_head_key",q=>q.eq("repositoryHead",repositoryHead)).collect();const now=Date.now();for(const t of rows)if(t.kind==="pr_review"&&["reviewing","publishing"].includes(t.state)&&!t.rerunCleanupPending)await db.patch(t._id,{state:"waiting_for_ci",leaseToken:undefined,updatedAt:now});const cleanup=[];for(const t of rows.filter(t=>t.kind==="pr_review"&&(t.state==="completed"||t.rerunCleanupPending))){const c:any=await db.get(t.conversationId);if(!c?.pullRequestNumber)continue;if(t.rerunCleanupPending&&t.rerunResultState){cleanup.push({id:t.externalId,pull_request_number:c.pullRequestNumber,result_state:t.rerunResultState});continue;}const peers:any[]=await db.query("tasks").withIndex("by_conversation_head_key",q=>q.eq("conversationHead",t.conversationHead)).collect();const hasActive=peers.some(p=>p._id!==t._id&&p.kind==="pr_review"&&active(p.state));const resultState=hasActive?"superseded":"reopened";await db.patch(t._id,{state:hasActive?"superseded":"waiting_for_ci",leaseToken:undefined,rerunCleanupPending:true,rerunResultState:resultState,updatedAt:now});cleanup.push({id:t.externalId,pull_request_number:c.pullRequestNumber,result_state:resultState});}return cleanup;},
});
export const acknowledgeRerunCleanup = mutationGeneric({args:{secret:v.string(),taskId:v.string()},handler:async({db},a)=>{authorize(a.secret);const t:any=await db.query("tasks").withIndex("by_external_id",q=>q.eq("externalId",a.taskId)).unique();if(t?.rerunCleanupPending)await db.patch(t._id,{rerunCleanupPending:undefined,rerunResultState:undefined,updatedAt:Date.now()});return null;}});
