...
export const completeCiTask = mutationGeneric({
  args: { secret: v.string(), repositoryId: v.string(), pullRequestNumber: v.number(), headSha: v.string(), taskId: v.string(), leaseToken: v.string(), conclusion: v.union(v.literal("success"), v.literal("failure")) },
  handler: async ({ db }, a) => {
    authorize(a.secret);
    const t: any = await db.query("tasks").withIndex("by_external_id", q => q.eq("externalId", a.taskId)).unique();
    if (!t || t.repositoryId !== a.repositoryId || t.headSha !== a.headSha || t.state !== "publishing" || t.leaseToken !== a.leaseToken) return false;
    const c: any = await db.get(t.conversationId);
    if (c?.pullRequestNumber !== a.pullRequestNumber) return false;
    await db.patch(t._id, { state: "completed", claimState: "completed", ciConclusion: a.conclusion, updatedAt: Date.now() });
    return true;
  },
});

export const claimResultPost = mutationGeneric({args:{secret:v.string(),taskId:v.string(),leaseToken:v.string()},handler:async({db},a)=>{authorize(a.secret);const t:any=await db.query("tasks").withIndex("by_external_id",q=>q.eq("externalId",a.taskId)).unique();if(!t||t.state!=="publishing"||t.leaseToken!==a.leaseToken)return{claimed:false,comment_id:t?.resultCommentId??null};if(t.resultPostClaim&&t.resultPostClaim!==a.leaseToken&&(t.resultPostClaimAt??0)>Date.now()-15*60_000)return{claimed:false,comment_id:t.resultCommentId??null};await db.patch(t._id,{resultPostClaim:a.leaseToken,resultPostClaimAt:Date.now(),updatedAt:Date.now()});return{claimed:true,comment_id:t.resultCommentId??null};}});
export const recordResultComment = mutationGeneric({args:{secret:v.string(),taskId:v.string(),leaseToken:v.string(),commentId:v.number(),body:v.string()},handler:async({db},a)=>{authorize(a.secret);const t:any=await db.query("tasks").withIndex("by_external_id",q=>q.eq("externalId",a.taskId)).unique();if(!t||t.resultPostClaim!==a.leaseToken)return false;await db.patch(t._id,{resultCommentId:a.commentId,resultCommentBody:a.body,resultPostClaim:undefined,resultPostClaimAt:undefined,updatedAt:Date.now()});return true;}});
export const replaceMissingResultComment = mutationGeneric({args:{secret:v.string(),taskId:v.string(),leaseToken:v.string(),commentId:v.number()},handler:async({db},a)=>{authorize(a.secret);const t:any=await db.query("tasks").withIndex("by_external_id",q=>q.eq("externalId",a.taskId)).unique();if(!t||t.state!=="publishing"||t.leaseToken!==a.leaseToken||t.resultCommentId!==a.commentId)return false;await db.patch(t._id,{resultCommentId:undefined,resultPostClaim:a.leaseToken,resultPostClaimAt:Date.now(),updatedAt:Date.now()});return true;}});

export const claimDeferred = mutationGeneric({
  args: { secret: v.string(), limit: v.number(), staleBefore: v.number() },
  handler: async ({ db }, a) => { authorize(a.secret); const waiting: any[] = await db.query("tasks").withIndex("by_claim_state_updated",q=>q.eq("claimState","waiting_for_ci")).take(a.limit); const reviewing: any[] = (await db.query("tasks").withIndex("by_claim_state_updated",q=>q.eq("claimState","reviewing")).take(a.limit)).filter((t:any)=>t.updatedAt<a.staleBefore); const publishing: any[] = (await db.query("tasks").withIndex("by_claim_state_updated",q=>q.eq("claimState","publishing")).take(a.limit)).filter((t:any)=>t.updatedAt<a.staleBefore); const held: any[] = (await db.query("tasks").withIndex("by_claim_state_updated",q=>q.eq("claimState","rerun_held")).take(a.limit)).filter((t:any)=>t.updatedAt<a.staleBefore); const candidates=[...waiting,...reviewing,...publishing,...held].filter(t=>t.headSha).sort((x,y)=>x.updatedAt-y.updatedAt||x._creationTime-y._creationTime).slice(0,a.limit); const out=[]; for (const t of candidates) { const c: any = await db.get(t.conversationId); if (!c?.repositoryId || !c.repositoryOwner || !c.repositoryName || !c.pullRequestNumber) continue; const leaseToken=crypto.randomUUID(); await db.patch(t._id,{state:"reviewing",claimState:"reviewing",rerunHold:undefined,leaseToken,updatedAt:Date.now()}); out.push({id:t.externalId,head_sha:t.headSha,repository_id:c.repositoryId,repository_owner:c.repositoryOwner,repository_name:c.repositoryName,github_installation_id:c.githubInstallationId??null,pull_request_number:c.pullRequestNumber,lease_token:leaseToken,result_comment_id:t.resultCommentId??null}); } return out; },
});

export const claimCompletedForRevalidation = mutationGeneric({
  args: { secret: v.string(), limit: v.number() },
  handler: async ({ db }, a) => {
    authorize(a.secret);
    const rows: any[] = await db.query("tasks")
      .withIndex("by_claim_state_updated", q => q.eq("claimState", "completed"))
      .take(a.limit);
    const out = [];
    for (const t of rows) {
      if (t.kind !== "pr_review" || t.state !== "completed" || !t.headSha) continue;
      const c: any = await db.get(t.conversationId);
      if (!c?.repositoryId || !c.pullRequestNumber) continue;
      await db.patch(t._id, { updatedAt: Date.now() });
      out.push({
        id: t.externalId, head_sha: t.headSha, repository_id: c.repositoryId,
        repository_owner: c.repositoryOwner, repository_name: c.repositoryName,
        github_installation_id: c.githubInstallationId ?? null,
        pull_request_number: c.pullRequestNumber, ci_conclusion: t.ciConclusion ?? null,
      });
    }
    return out;
  },
...
export const claimRerunCleanup = mutationGeneric({
  args: { secret: v.string(), limit: v.number() },
  handler: async ({ db }, a) => {
    authorize(a.secret);
    const rows: any[] = await db.query("tasks").withIndex("by_claim_state_updated", q => q.eq("claimState", "rerun_cleanup")).take(a.limit);
    const out = [];
    for (const t of rows) {
      if (!t.rerunCleanupPending || !t.rerunResultState) continue;
      const c: any = await db.get(t.conversationId);
      if (!c?.repositoryId || !c.pullRequestNumber) continue;
      await db.patch(t._id, { updatedAt: Date.now() });
      out.push({ id: t.externalId, head_sha: t.headSha, repository_id: c.repositoryId, repository_owner: c.repositoryOwner, repository_name: c.repositoryName, github_installation_id: c.githubInstallationId ?? null, pull_request_number: c.pullRequestNumber, result_state: t.rerunResultState, result_comment_id: t.resultCommentId ?? null });
    }
    return out;
  },
});
...
export const moveOperationBranch = mutationGeneric({args:{secret:v.string(),id:v.string(),expectedBranch:v.string(),nextBranch:v.string()},handler:async({db},a)=>{authorize(a.secret);const o:any=await db.query("changeOperations").withIndex("by_external_id",q=>q.eq("externalId",a.id)).unique();if(!o||o.branch!==a.expectedBranch||o.pullRequestNumber!==undefined)return false;await db.patch(o._id,{branch:a.nextBranch,updatedAt:Date.now()});return true;}});

export const holdRerun = mutationGeneric({
  args:{secret:v.string(),repositoryId:v.string(),headSha:v.string()},
  handler:async({db},a)=>{authorize(a.secret);const repositoryHead=`${a.repositoryId}\u0000${a.headSha}`;const rows:any[]=await db.query("tasks").withIndex("by_repository_head_key",q=>q.eq("repositoryHead",repositoryHead)).collect();const now=Date.now();const pulls=new Set<number>();for(const t of rows){if(t.kind!=="pr_review")continue;const c:any=await db.get(t.conversationId);if(c?.pullRequestNumber)pulls.add(c.pullRequestNumber);if(["queued","waiting_for_ci","reviewing","publishing","waiting_for_user"].includes(t.state))await db.patch(t._id,{state:"waiting_for_ci",claimState:t.rerunCleanupPending?"rerun_cleanup":t.state==="waiting_for_ci"?"waiting_for_ci":t.state==="publishing"?"rerun_cleanup":"rerun_held",leaseToken:undefined,rerunHold:t.state==="waiting_for_ci"?undefined:true,resultPostClaim:undefined,resultPostClaimAt:undefined,...(t.state==="publishing"&&!t.rerunCleanupPending?{rerunCleanupPending:true,rerunResultState:"reopened"}:{}),updatedAt:now});}return [...pulls];},
});
export const resolveRerunPull = mutationGeneric({args:{secret:v.string(),repositoryId:v.string(),pullRequestNumber:v.number(),headSha:v.string(),disposition:v.union(v.literal("valid"),v.literal("cancelled"),v.literal("superseded"))},handler:async({db},a)=>{authorize(a.secret);const repositoryPull=`${a.repositoryId}\u0000${a.pullRequestNumber}`;const rows:any[]=(await db.query("tasks").withIndex("by_repository_pull",q=>q.eq("repositoryPull",repositoryPull)).collect()).filter((t:any)=>t.headSha===a.headSha&&t.kind==="pr_review");const now=Date.now();const cleanup=[];for(const t of rows){const item=(result_state:"reopened"|"superseded"|"cancelled")=>({id:t.externalId,pull_request_number:a.pullRequestNumber,result_state,result_comment_id:t.resultCommentId??null});if(t.rerunCleanupPending&&t.rerunResultState){if(a.disposition!=="valid"&&t.rerunResultState!==a.disposition){await db.patch(t._id,{state:a.disposition,claimState:"rerun_cleanup",rerunHold:undefined,leaseToken:undefined,rerunResultState:a.disposition,updatedAt:now});cleanup.push(item(a.disposition));}else{await db.patch(t._id,{claimState:"rerun_cleanup",updatedAt:now});cleanup.push(item(t.rerunResultState));}continue;}if(a.disposition!=="valid"){if(t.state==="completed"){await db.patch(t._id,{state:a.disposition,claimState:"rerun_cleanup",rerunHold:undefined,rerunCleanupPending:true,rerunResultState:a.disposition,updatedAt:now});cleanup.push(item(a.disposition));}else if(active(t.state))await db.patch(t._id,{state:a.disposition,claimState:undefined,rerunHold:undefined,leaseToken:undefined,updatedAt:now});continue;}if(t.state!=="completed")continue;const peers:any[]=await db.query("tasks").withIndex("by_conversation_head_key",q=>q.eq("conversationHead",t.conversationHead)).collect();const hasActive=peers.some(p=>p._id!==t._id&&p.kind==="pr_review"&&active(p.state));const resultState=hasActive?"superseded":"reopened";await db.patch(t._id,{state:hasActive?"superseded":"waiting_for_ci",claimState:"rerun_cleanup",leaseToken:undefined,rerunHold:hasActive?undefined:true,rerunCleanupPending:true,rerunResultState:resultState,updatedAt:now});cleanup.push(item(resultState));}return cleanup;}});
export const acknowledgeRerunCleanup = mutationGeneric({args:{secret:v.string(),taskId:v.string(),expectedResultState:v.union(v.literal("reopened"),v.literal("superseded"),v.literal("cancelled"))},handler:async({db},a)=>{authorize(a.secret);const t:any=await db.query("tasks").withIndex("by_external_id",q=>q.eq("externalId",a.taskId)).unique();const expectedState=a.expectedResultState==="reopened"?"waiting_for_ci":a.expectedResultState;if(!t?.rerunCleanupPending||t.rerunResultState!==a.expectedResultState||t.state!==expectedState)return false;await db.patch(t._id,{rerunCleanupPending:undefined,rerunResultState:undefined,claimState:t.rerunHold?"rerun_held":t.state,updatedAt:Date.now()});return true;}});
export const rerunDisposition = queryGeneric({args:{secret:v.string(),taskId:v.string()},handler:async({db},a)=>{authorize(a.secret);const t:any=await db.query("tasks").withIndex("by_external_id",q=>q.eq("externalId",a.taskId)).unique();if(!t)return null;return{result_state:t.state==="cancelled"?"cancelled":t.state==="superseded"?"superseded":t.state==="completed"?"completed":t.state==="publishing"?"publishing":"reopened",conclusion:t.ciConclusion??null,body:t.resultCommentBody??null,comment_id:t.resultCommentId??null,lease_token:t.leaseToken??null};}});
export const supersedeCompleted = mutationGeneric({args:{secret:v.string(),taskId:v.string()},handler:async({db},a)=>{authorize(a.secret);const t:any=await db.query("tasks").withIndex("by_external_id",q=>q.eq("externalId",a.taskId)).unique();if(t?.state==="completed")await db.patch(t._id,{state:"superseded",claimState:undefined,leaseToken:undefined,updatedAt:Date.now()});return null;}});
export const retireTask = mutationGeneric({args:{secret:v.string(),taskId:v.string()},handler:async({db},a)=>{authorize(a.secret);const t:any=await db.query("tasks").withIndex("by_external_id",q=>q.eq("externalId",a.taskId)).unique();if(t)await db.patch(t._id,{state:"cancelled",claimState:undefined,leaseToken:undefined,rerunHold:undefined,rerunCleanupPending:undefined,rerunResultState:undefined,updatedAt:Date.now()});return null;}});
export const cancelPullTasks = mutationGeneric({args:{secret:v.string(),repositoryId:v.string(),pullRequestNumber:v.number()},handler:async({db},a)=>{authorize(a.secret);const repositoryPull=`${a.repositoryId}\u0000${a.pullRequestNumber}`;const rows:any[]=await db.query("tasks").withIndex("by_repository_pull",q=>q.eq("repositoryPull",repositoryPull)).collect();for(const t of rows){if(t.kind!=="pr_review"||!["queued","waiting_for_ci","reviewing","waiting_for_user"].includes(t.state))continue;await db.patch(t._id,{state:"cancelled",claimState:t.rerunCleanupPending?"rerun_cleanup":undefined,rerunHold:undefined,...(t.rerunCleanupPending?{rerunResultState:"cancelled"}:{}),updatedAt:Date.now()});}return null;}});
...
export const supersedeOldHeads = mutationGeneric({args:{secret:v.string(),repositoryId:v.string(),pullRequestNumber:v.number(),headSha:v.string()},handler:async({db},a)=>{authorize(a.secret);const repositoryPull=`${a.repositoryId}\u0000${a.pullRequestNumber}`;const rows:any[]=await db.query("tasks").withIndex("by_repository_pull",q=>q.eq("repositoryPull",repositoryPull)).collect();const completed=[];for(const t of rows){if(t.kind!=="pr_review"||t.headSha===a.headSha)continue;if(t.state==="completed")completed.push({id:t.externalId,head_sha:t.headSha,result_comment_id:t.resultCommentId??null});else if(t.state==="publishing")await db.patch(t._id,{state:"superseded",claimState:"rerun_cleanup",leaseToken:undefined,rerunHold:undefined,rerunCleanupPending:true,rerunResultState:"superseded",resultPostClaim:undefined,resultPostClaimAt:undefined,updatedAt:Date.now()});else if(["queued","waiting_for_ci","reviewing","waiting_for_user"].includes(t.state))await db.patch(t._id,{state:"superseded",claimState:t.rerunCleanupPending?"rerun_cleanup":undefined,rerunHold:undefined,...(t.rerunCleanupPending?{rerunResultState:"superseded"}:{}),updatedAt:Date.now()});}return completed;}});
