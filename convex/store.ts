...
export const recordResultComment = mutationGeneric({args:{secret:v.string(),taskId:v.string(),leaseToken:v.string(),commentId:v.number()},handler:async({db},a)=>{authorize(a.secret);const t:any=await db.query("tasks").withIndex("by_external_id",q=>q.eq("externalId",a.taskId)).unique();if(!t||t.resultPostClaim!==a.leaseToken)return false;await db.patch(t._id,{resultCommentId:a.commentId,resultPostClaim:undefined,resultPostClaimAt:undefined,updatedAt:Date.now()});return true;}});
