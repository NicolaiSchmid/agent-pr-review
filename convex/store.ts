...
export const claimResultPost = mutationGeneric({args:{secret:v.string(),taskId:v.string(),leaseToken:v.string()},handler:async({db},a)=>{authorize(a.secret);const t:any=await db.query("tasks").withIndex("by_external_id",q=>q.eq("externalId",a.taskId)).unique();if(!t||t.state!=="publishing"||t.leaseToken!==a.leaseToken)return{claimed:false,comment_id:t?.resultCommentId??null};if(t.resultCommentId!==undefined)return{claimed:true,comment_id:t.resultCommentId};if(t.resultPostClaim&&t.resultPostClaim!==a.leaseToken&&(t.resultPostClaimAt??0)>Date.now()-15*60_000)return{claimed:false,comment_id:null};await db.patch(t._id,{resultPostClaim:a.leaseToken,resultPostClaimAt:Date.now(),updatedAt:Date.now()});return{claimed:true,comment_id:null};}});
...
export const claimRerunCleanup = mutationGeneric({
  ...
  handler: async ({ db }, a) => {
    ...
    for (const t of rows) {
      ...
      if (!c?.repositoryId || !c.pullRequestNumber) continue;
      await db.patch(t._id, { updatedAt: Date.now() });
      out.push({ id: t.externalId, head_sha: t.headSha, repository_id: c.repositoryId, repository_owner: c.repositoryOwner, repository_name: c.repositoryName, github_installation_id: c.githubInstallationId ?? null, pull_request_number: c.pullRequestNumber, result_state: t.rerunResultState });
    }
    return out;
  },
});
